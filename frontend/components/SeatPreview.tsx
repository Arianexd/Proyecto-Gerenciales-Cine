...
export default function SeatPreview({ 
  ...
  // Determine hall size category
  const hallSize = hallCapacity < 100 ? 'small' : hallCapacity < 200 ? 'medium' : 'large';
  
  // Calculate seat position (left/center/right)
  ...
  // Calculate row position (front/middle/back)
  const getRowPosition = () => {
    if (!seatRow) return 'middle';
    const rowCode = seatRow.charCodeAt(0) - 65; // A=0, B=1, etc.
